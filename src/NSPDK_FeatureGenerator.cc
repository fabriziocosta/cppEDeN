...
void WDK_FeatureGenerator::generate_feature_vector(const GraphClass& aG, SVector& x, const vector<unsigned>& aFirstEndpointList) {
	...
	GetFirstEndpoints(aG, first_endpoint_list);
	if (first_endpoint_list.size() == 0)
		throw std::logic_error("ERROR:WDK_FeatureGenerator::generate_feature_vector: Something went wrong: cannot generate features over an empty set of first endpoints!");
	int horizon = max(mDistance, mRadius);
	aG.ComputePairwiseDistanceInformation(horizon, first_endpoint_list);
	if (aG.Check() == false)
		throw logic_error("ERROR:WDK_FeatureGenerator::generate_feature_vector: the graph data structure is not sound and it has not passed the checkup procedure"); //check graph data structure soundness
...
void WDK_FeatureGenerator::generate_vertex_feature_vector(const GraphClass& aG, vector<SVector>& x_list, const vector<unsigned>& aFirstEndpointList) {
	...
	GetFirstEndpoints(aG, first_endpoint_list);
	if (first_endpoint_list.size() == 0)
		throw std::logic_error("ERROR:WDK_FeatureGenerator::generate_vertex_feature_vector: Something went wrong: cannot generate features over an empty set of first endpoints!");
	int horizon = max(mDistance, mRadius);
	aG.ComputePairwiseDistanceInformation(horizon, first_endpoint_list);
	if (aG.Check() == false)
		throw logic_error("ERROR:WDK_FeatureGenerator::generate_vertex_feature_vector: the graph data structure is not sound and it has not passed the checkup procedure"); //check graph data structure soundness
...
void WDK_FeatureGenerator::GenerateVertexFeatures(unsigned aSrcID, const GraphClass& aG, unsigned aRadius, unsigned aDistance, SVector& x) {
	...
	unsigned src_code = HashFunc(aG.GetVertexLabelConcatenated(aSrcID), mHashBitMask);
	vector<unsigned> dest_id_list = aG.GetFixedDistanceVertexIDList(aSrcID, aDistance);
	for (unsigned dest_j = 0; dest_j < dest_id_list.size(); dest_j++) {
		unsigned dest_id = dest_id_list[dest_j];
		unsigned dest_code = 0;
		if (aG.GetVertexKernelPoint(dest_id) && aG.GetVertexAlive(dest_id)) { //proceed to extract features only if the *dest* vertex is a kernel point and is alive
			dest_code = HashFunc(aG.GetVertexLabelConcatenated(dest_id), mHashBitMask);
...
void WDK_FeatureGenerator::ReHash(SVector& x, unsigned aReHashCode) {
	...
	for (SVector::InnerIterator it(x); it; ++it) {
		...
		unsigned code = HashFunc(hash_vec, mHashBitMask);
		// NOTE: random insertions are expensive; collisions overwrite previous value:
		z.coeffRef(code) = val;
	}
	x = z;
}
...
void WDK_FeatureGenerator::GenerateVertexFeatures(unsigned aRootVertexIndex, const GraphClass& aG, unsigned aRadius, SVector& x) {
	if (mSparseFeatureFlagCache[aRootVertexIndex]) {
		...
		unsigned root_label_code = HashFunc(root_label_string, mHashBitMask);
		...
		for (unsigned u = 0; u < gal.VertexSize(); ++u) {
			...
			unsigned u_label_code = HashFunc(u_label_string, mHashBitMask);
			//get all edges of vertex u
			...
			for (unsigned j = 0; j < vertex_adjacency_list.size(); ++j) {
				...
				unsigned v_label_code = HashFunc(v_label_string, mHashBitMask);
				...
				unsigned hash_edge_encoding = HashFunc(edge_encoding, mHashBitMask);
				x.coeffRef(hash_edge_encoding) += 1;
			}
		}
...
void ANSPDK_FeatureGenerator::generate_feature_vector(const GraphClass& aG, SVector& x, const vector<unsigned>& aFirstEndpointList) {
	...
	GetFirstEndpoints(aG, first_endpoint_list);
	if (first_endpoint_list.size() == 0)
		throw std::logic_error("ERROR:ANSPDK_FeatureGenerator::generate_feature_vector: Something went wrong: cannot generate features over an empty set of first endpoints!");
	int horizon = max(mDistance, mRadius);
	aG.ComputePairwiseDistanceInformation(horizon, first_endpoint_list);
	if (aG.Check() == false)
		throw logic_error("ERROR:ANSPDK_FeatureGenerator::generate_feature_vector: the graph data structure is not sound and it has not passed the checkup procedure"); //check graph data structure soundness
		...
		for (unsigned d = 0; d <= mDistance; d++) {
			...
			for (unsigned i = 0; i < first_endpoint_list.size(); i++) {
				...
				if (aG.GetVertexViewPoint(src_id) && aG.GetVertexKernelPoint(src_id) && aG.GetVertexAlive(src_id)) { //proceed to extract features only if the *src* vertex is a kernel point and is alive
					...
					SVector zv(pow(2, mHashBitSize));
					GenerateAbstractVertexFeatures(src_id, aG, r, d, zv);
					z += zv;
				}
			}
			if (mNormalization)
...
void ANSPDK_FeatureGenerator::generate_vertex_feature_vector(const GraphClass& aG, vector<SVector>& x_list, const vector<unsigned>& aFirstEndpointList) {
	...
	GetFirstEndpoints(aG, first_endpoint_list);
	if (first_endpoint_list.size() == 0)
		throw std::logic_error("ERROR:ANSPDK_FeatureGenerator::generate_vertex_feature_vector: Something went wrong: cannot generate features over an empty set of first endpoints!");
	int horizon = max(mDistance, mRadius);
	aG.ComputePairwiseDistanceInformation(horizon, first_endpoint_list);
	if (aG.Check() == false)
		throw logic_error("ERROR:ANSPDK_FeatureGenerator::generate_vertex_feature_vector: the graph data structure is not sound and it has not passed the checkup procedure"); //check graph data structure soundness
...
//ensure that the src vertex is of the abstraction type
	if (aG.GetVertexAbstraction(aSrcID) == false)
		throw std::logic_error("ERROR:ANSPDK_FeatureGenerator::GenerateAbstractVertexFeatures: Something went wrong: expecting an abstraction vertex, but abstraction test fails.");
//extract all adjacent vertices and partition them into part_of (lower level) and abstraction_of (upper level)
	...
	if (vertex_adjacency_list.size() != edge_adjacency_list.size())
		throw std::logic_error("ERROR:ANSPDK_FeatureGenerator::GenerateAbstractVertexFeatures: Something went wrong: expecting vertex adjacency list to be the same size as the edge adjacency list.");
	if (vertex_adjacency_list.size() == 0)
		throw std::logic_error("ERROR:ANSPDK_FeatureGenerator::GenerateAbstractVertexFeatures: Something went wrong: expecting a non empty vertex adjacency list for each abstract vertex.");
	for (unsigned i = 0; i < vertex_adjacency_list.size(); ++i) {
		unsigned child_vertex_id = vertex_adjacency_list[i];
		unsigned child_edge_id = edge_adjacency_list[i];
...
//starting from each upper level vertex find all vertices at distance aDistance
	if (abstraction_of_list.size() == 0)
		throw std::logic_error("ERROR:ANSPDK_FeatureGenerator::GenerateAbstractVertexFeatures: Something went wrong: expecting a non empty abstraction_of list.");
...
//starting from each lower level vertex find all vertices at distance aDistance
	if (part_of_list.size() == 0)
		throw std::logic_error("ERROR:ANSPDK_FeatureGenerator::GenerateAbstractVertexFeatures: Something went wrong: expecting a non empty part_of list.");
